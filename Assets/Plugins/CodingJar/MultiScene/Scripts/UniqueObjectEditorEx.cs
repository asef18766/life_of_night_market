		...
		internal Object	EditorResolveSlow()
		{
			...
			foreach( var obj in allObjs )
			{
				...
				if ( prefabObj && editorLocalId == GetEditorId(prefabObj) )
				{
					...
					Debug.LogWarningFormat( gameObject, "Performed a slow resolve on {0} due to a rename.  We found a PREFAB with same ID named {1} (but we're not sure). Attempting a resolve with it.", this, gameObject );
