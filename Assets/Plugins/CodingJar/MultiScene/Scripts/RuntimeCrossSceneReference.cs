				...
				if ( !_toObjectCached )
				{
					...
					if ( !_toObjectCached )
					{
						...
						if ( _toObjectCached )
						{
							...
							var scene = gameObject ? gameObject.scene : default(UnityEngine.SceneManagement.Scene);
							if ( scene.isLoaded && !scene.isDirty )
							{
								UnityEditor.SceneManagement.EditorSceneManager.MarkSceneDirty( scene );
								UnityEditor.EditorApplication.delayCall += () => { AmsDebug.LogWarning( gameObject, "Scene {0} needs to be resaved due to AMS entries being moved (see warnings above)", scene.name ); };
							}
						}
					}
#endif
